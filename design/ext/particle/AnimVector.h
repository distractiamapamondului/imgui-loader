...
float glass_opticaly = 90.0f;
float vector_speed = 1.0f;
...
void dot::update() {
	auto opacity = 150.0f / 150.0f;

	m_pos += m_vel * (opacity);
}

void dot::draw() {
	int opacity = 150.0f * (150.0f / 150.0f);
	ImGui::GetWindowDrawList()->AddRectFilled({ m_pos.x - 2.5f, m_pos.y - 2.5f }, { m_pos.x + 2.5f, m_pos.y + 2.5f }, ImColor(90,90,90), 30);
	...
	for (auto i = t; i != dots.end(); i++) {
		...
		if (dist < glass_opticaly) {
			...
			ImGui::GetWindowDrawList()->AddLine({ m_pos.x - 2, m_pos.y - 3 }, { (*i)->m_pos.x - 2, (*i)->m_pos.y - 3 }, ImColor( 50, 50, 50 ) );
		}
	}
}
...
void dot_draw() {
	...
	if (GetTickCount() > 1.0f + (1 * 2))
	{
		...
		if (s == 0) {
			...
			dots.push_back(new dot(SourceEngine::Vector(rand() % (int)sc.x, (int)sc.y + 10, 0), SourceEngine::Vector((rand() % 7) - 3, -1 * (rand() % 3 + 1), 0)));
		}
		else if (s == 2) {
			dots.push_back(new dot(SourceEngine::Vector(-5, rand() % (int)sc.y, 0), SourceEngine::Vector(rand() % 3 + 1, (rand() % 7) - 3, 0)));
		}
		else if (s == 3) {
			dots.push_back(new dot(SourceEngine::Vector((int)sc.x + 2, rand() % (int)sc.y, 0), SourceEngine::Vector(-1 * (rand() % 3 + 1), (rand() % 7) - 3, 0)));
		}
